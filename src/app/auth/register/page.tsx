...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) toast.error(data.error || "Registrasi gagal");
      else { toast.success("Akun berhasil dibuat!"); router.push("/auth/login"); }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12" style={{ backgroundColor: "var(--bg-base)" }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Buat Akun Baru</h1>
          <p className="mt-1 text-sm" style={{ color: "var(--text-secondary)" }}>Gratis dan tanpa biaya tersembunyi</p>
        </div>

        <div className="card p-8">
          {/* Google */}
          <button onClick={handleGoogle} disabled={googleLoading || loading}
            className="w-full flex items-center justify-center gap-3 font-medium py-2.5 px-4 rounded-lg border
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" className="btn-primary w-full py-2.5 mt-2" disabled={loading || googleLoading}>
              {loading ? (
        ...
        <p className="text-center mt-6">
          <Link href="/" className="text-sm" style={{ color: "var(--text-muted)" }}>← Kembali ke Beranda</Link>
        </p>
      </div>
    </div>
  );
